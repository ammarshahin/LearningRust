...
pub fn run() {
  ...
  let v2 = vec![1, 2, 3]; // create a vector using the macro declaration
  ...
  //     Some(&i) => i,
  //     None => panic!("invalid access !!!!"),
  // };

  let valid_access = v1.get(2).expect("invalid access !!!!");
  ...
  // this is useful when we don't care about a collection any more and we only interested in the elements it self
